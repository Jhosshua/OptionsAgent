(() => {
  ...
  async function load() { try { const [summary, trades, positions, research, risk, system] = await Promise.all([api("/api/summary"), api("/api/trades"), api("/api/positions"), api("/api/research"), api("/api/risk"), api("/api/system")]); renderSummary(summary); renderHistory(trades.trades); renderBars(trades.daily_pnl || []); renderPositions(positions.positions); renderResearch(research); renderRisk(risk); renderSystem(system); } catch (error) { $("warning").hidden = false; $("warning-title").textContent = "Dashboard data unavailable"; $("warning-detail").textContent = "The server is alive but its cached data could not be read."; } }
